...
import org.hs_coburg.lemos.general.Configuration;
...
@JsonSubTypes({@Type(name = "TEXT", value = TextField.class),
			   @Type(name = "PLAIN", value = PlainField.class),
...
public abstract class Field
{
	...
	protected String performReplacements(String string)
	{
		return string.replace("{{id}}", StringHelper.escape(id))
					 ...
					 .replace("{{style}}", style.generateFieldStyleCSS())
	...
	{
		if (Configuration.get().showFieldIDs && !name.equals(id))
		{
			return name + " (" + id + ")";
		}
		else
